...
-- ================================================================
-- Internal reference catalog for staff preparing quotations manually —
-- specs, HS code, default cost components, per-supplier sourcing/FOB, and
-- misc reference charges. Deliberately standalone: never selected into
-- orders/order_products, and nothing here is ever shown to clients.
--
...
-- unrelated concept, so it uses catalog_products/catalog_product_images/
-- catalog_product_suppliers/catalog_product_misc_charges to avoid any
-- collision with that existing table.
...
-- not a constraint.
--
-- is_enabled cannot be set 0->... wait, cannot be flipped 1->0 while any
-- is_test_data=1 row exists anywhere (TestModeService.disable() enforces
-- this in application code, not a DB trigger, matching this codebase's
-- convention of keeping business rules in the service layer). test_email
-- is editable at any time, on or off, so it's ready before the next time
...
-- fields (userController's update/toggleActive/forceResetPassword,
-- superAdminService.setPermanentFlag, signatoryController's
-- setEligibility) and the trigger below is the real, unbypassable
-- backstop — it fires regardless of which code path or DB user issues the
-- statement, exactly like Section N's trg_users_super_admin_bd/bu.
--
